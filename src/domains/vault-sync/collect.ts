...
function hashContent(frontmatterRaw: string, body: string, tags: string[]): string {
  return sha256Hex(`${frontmatterRaw}\n${body}\n${tags.slice().sort().join(",")}`);
}

async function collectFiles(vault: string, pattern: string, excludes: Glob[]): Promise<string[]> {
...
export async function collectVaultDocuments(): Promise<Map<string, VaultDocument>> {
  const vault = vaultDir();
  const config = await loadSyncConfig();
  const excludes = (config.defaults.exclude ?? []).map((pattern) => new Glob(pattern));
  ...
  for (const source of config.sources) {
    ...
    for (const rel of files) {
      ...
      for (const [k, v] of Object.entries(source.tags ?? {})) tags.add(`${k}:${v}`);
      for (const key of promoteKeys) {
        const val = frontmatter[key];
        if (typeof val === "string" && val.trim()) tags.add(`${key}:${val.trim()}`);
        else if (Array.isArray(val))
          for (const v of val)
            if (typeof v === "string" && v.trim()) tags.add(`${key}:${v.trim()}`);
      }
