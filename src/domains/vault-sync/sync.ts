...
export async function syncProfile(opts: { dryRun?: boolean } = {}): Promise<SyncResult> {
  ...
  if (added + updated > 0) {
    console.log([...plan.toAdd, ...plan.toUpdate].map((d) => `  ${d.documentId}`).join("\n"));
  }
  if (purged > 0) console.log(plan.toPurge.map((id) => `  purge ${id}`).join("\n"));
